...
use crate::sim::crafting::{CraftRequest, CurrentRecipe, Inputs, MachineRecipeChanged};
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SECTION 1 – CONSTANTS & MARKERS
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const SPATIAL_ITEM_ICON_SIZE: f32 = 64.0;
const OUTPUT_GHOST_SLOT_SIZE: Val = Val::Px(64.0);
...
/// Marker on each item child inside a spatial inventory area. The sync
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SECTION 2 – PANEL & PLACEMENT BUILDERS
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
/// Build the crafting area panel. Items are added as absolutely-positioned
/// children by the sync observer as placements happen.
...
///
/// `pos` represents the CENTER of the icon, so we offset by half-size when
/// computing `left`/`top` — this feels more natural with freeform placement.
fn build_placement_node(
    ...
    (
        ...
        // Placements don't catch clicks; the spatial inventory panel
        // hit-tests them against the data. They're a mirage.
        Pickable::IGNORE,
        children![build_ui_item_display(
...
fn spatial_inventory_click_obs(
    ...
    let Some(hit_position) = click.hit.position else { return };

    // Pointer hit → panel-local pixels.
    let local_pos = hit_position.truncate() * panel.size() + panel.size() * 0.5;
...
fn spatial_inventory_changed_to_ui_sync_obs(
    ...
    match event.change {
        SpatialChange::Placed(id) => {
            ...
            for (area_entity, area) in areas.iter() {
                ...
                let child = commands
                    .spawn(build_placement_node(target, id, placement, &item_registry))
                    .id();
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SECTION 5 – THE GHOST OUTPUT
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// A pure VIEW of the machine's `CurrentRecipe` cache. It stores nothing —
/// clicking it asks the machine to craft.
#[derive(Component)]
...
/// `MachineRecipeChanged` → redraw every ghost slot viewing that machine.
fn recipe_output_sync_obs(
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SECTION 6 – RECIPE SHAPE OVERLAY
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
/// A freshly spawned overlay requests its own first sync — same pattern
...
fn recipe_shape_overlay_sync_obs(
    event: On<MachineRecipeChanged>,
    mut commands: Commands,
    ...
    for (overlay_entity, overlay) in overlays.iter() {
        ...
        let positions: Option<Vec<Vec2>> = recipe
            .consume
            .iter()
...
impl Plugin for UiCraftingPlugin {
    fn build(&self, app: &mut App) {
        app.add_observer(spatial_inventory_click_obs)
