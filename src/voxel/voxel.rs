...
/// ## What the fields buy
///
/// **`shape` is not stored.** It used to occupy four bits so the mesher
/// could pick geometry without consulting the registry. That saved nothing:
/// the mesher already does `registry.get(block_id)` for every non-air voxel
/// to fetch appearance and collision, so the definition is in cache by the
...
/// purely an authoring-time input to the model baker, and a half-slab is
/// its own `BlockID`.
///
/// **`rotation` is 5 bits, not a 3-bit facing.** Six facings could only say
/// "point this shape at that wall", which is why `Stair` and `StairInv` had
/// to be separate shapes. The full 24-element rotation group of the cube
/// makes orientation a property of the voxel rather than of the shape
/// catalogue, so upside-down stairs, horizontal slopes and sideways panels
/// all come out of one canonical model for free.
///
/// **`state` is 11 bits.** A pipe spends six on its connection mask and has
/// five left over.
///
/// A raw value of `0` is unconditionally **air**, so empty chunks stay
/// trivially cheap and rotation 0 is the identity.
...
impl Voxel {
    ...
    ///
    /// Worth its own name because the voxel-write observer needs to
    /// distinguish it: a state-only edit must dirty the mesh **without**
    /// decomposing into Break + Place, or reconnecting a pipe would tear
    /// down its block entity and take its network membership with it.
    #[inline]
...
// ---------------------------------------------------------------------------
// NOTE: there is deliberately no `covers_face` here.
//
// Occlusion is a property of the *model*, derived at bake time by
