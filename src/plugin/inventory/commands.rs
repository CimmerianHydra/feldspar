...
use crate::plugin::console::registry::{CommandLevel, CommandSpec};
use crate::plugin::inventory::main::{insert_and_notify, Inventory};
use crate::plugin::inventory::player::PlayerInventories;
...
/// command handler is its input and output — which is exactly the point.
pub fn give_cmd(
    ...
    for target in [inventory_set.hotbar, inventory_set.main] {
        ...
        // `remainder`: see the note below about `transfer_items`.
...
///
/// Leans on item IDs being dense from 0, which `register` guarantees. An
/// `iter()` on `ItemRegistry` would be tidier, and would also serve tab
/// completion later — worth adding when there's a second caller.
fn suggest_item_names(registry: &ItemRegistry, needle: &str) -> Vec<String> {
    ...
    for index in 0..registry.len() {
        let name = &registry.get(ItemID(index as u16)).name;
