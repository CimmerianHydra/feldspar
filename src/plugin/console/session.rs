...
use crate::plugin::console::command::{CommandSource, PendingCommands};
use crate::plugin::controller::player::Player;
use crate::plugin::ui::screen::{TextFocus, UiScreenCommandsExt};
...
/// Same shape as `append_player_inventory_sys`: the console module hangs its own
/// state off the player without the controller having to know it exists.
...
/// two-frame backlog when it resumes — the `\` that opened the console among
