...
/// again at runtime.
///
/// The one meaningful change from the old `Quad`: `texture_direction:
/// Direction` is gone, replaced by `slot`. A model no longer knows
/// anything about textures — it knows it has N paintable surfaces. The
/// `BlockDefinition` decides what goes on them.
///
...
///
/// Triangular faces (slope sides) still use the degenerate-quad
/// convention: `verts[3] == verts[2]`, producing one real triangle and one
/// zero-area triangle the GPU discards.
...
///
/// Boxes cover everything you have described except slopes, which carry a
/// diagonal and therefore need an escape hatch. Keeping `Raw` narrow is
/// deliberate: the more geometry that goes through `Box`, the more of it
/// gets free colliders, free UVs and free cull tags.
#[derive(Clone, Debug)]
pub enum Element {
    Box(ElementBox),
...
/// `Some(dir)` iff this face is flush with the voxel boundary.
...
/// (1,0) — the same convention the existing `quads.rs` uses — with CCW
